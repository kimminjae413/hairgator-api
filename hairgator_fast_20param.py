...
async def generate_fast_20param_response(messages: List[ChatMessage], claude_analysis: str = None, rag_context: str = None) -> str:
    ...
    try:
        ...
        # RAG 기반 프롬프트 (문제 2 해결)
        prompt_base = f"""당신은 헤어게이터 전문가입니다. 아래 RAG 데이터베이스의 정보를 **최우선**으로 활용하여 자연스럽고 친근한 톤으로 답변해주세요.
...
위 RAG 데이터의 정확한 내용을 기반으로 다음 형식으로 자연스럽게 답변하세요:
...
마지막에 "더 궁금한 점이 있으시면 편하게 물어보세요! 😊" 같은 친근한 멘트를 자연스럽게 추가해주세요.

**중요: 모든 영어 용어를 한글로 완전 번역하고, RAG 데이터가 있다면 그 내용을 최우선으로 반영하세요.**
        ...
        response = await asyncio.wait_for(
            openai.ChatCompletion.acreate(
                ...
                messages=[
                    ...
                    {
                        "role": "user", 
                        "content": f"RAG 데이터베이스 기반으로 20파라미터 헤어 레시피를 모든 영어를 한글로 번역해서 자연스럽고 친근하게 알려주세요: {last_message}"
                    }
                ],
                max_tokens=1000,
                temperature=0.1,
            ...
            if similar_styles:
                ...
                for i, style in enumerate(similar_styles[:3]):  # 최대 3개
                    ...
                    if style.get('ground_truth'):
                        # Ground Truth 일부 포함 (너무 길어지지 않게)
                        truth_preview = style.get('ground_truth', '')[:200] + "..."
                        rag_context += f"레시피 샘플: {truth_preview}\n"
                    rag_context += "\n" + "-"*30 + "\n"
